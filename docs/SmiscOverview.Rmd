...
The `Smisc` package is a collection of functions for statistical computing, data manipulation, and visualization that I created to 
...
Sometimes it's handy to know whether all the elements in a given row (or column) of a matrix or data frame are missing.  `r rdl("allMissing()")` returns a logical vector indicating whether all the elements in a row (or column) are missing.  It's a nice compliment to [complete.cases()](https://www.rdocumentation.org/packages/stats/topics/complete.cases).
...
Have you ever needed to row bind, [rbind()](https://www.rdocumentation.org/packages/base/topics/cbind), or column bind, [cbind()](https://www.rdocumentation.org/packages/base/topics/cbind), or combine/concatenate, [c()](https://www.rdocumentation.org/packages/base/topics/c), *numerous* objects, usually of the same type?  If you do it iteratively, you'll slow your machine to a crawl:
...
`r rdl("df2list()")` converts a dataframe to a list, in a row-wise fashion (as opposed to [as.list()](https://www.rdocumentation.org/packages/base/topics/list), which converts in a column-wise fashion):
...
I use `r rdl("list2df()")` probably more than any other `Smisc` function. It converts a list of suitable object types (vectors, dataframes, or lists) into a single dataframe.  It's especially useful for collecting the output from [strsplit()](https://www.rdocumentation.org/packages/base/topics/strsplit), or the results from [lapply()](https://www.rdocumentation.org/packages/base/topics/lapply).
...
Naively attempting to convert a factor to a numeric vector can easily go wrong.  For example,  [as.numeric()](https://www.rdocumentation.org/packages/base/topics/as.numeric) probably won't give you what you're looking for. `r rdl("factor2numeric()")` correctly converts a factor to a numeric vector.
...
When writing a script for analysis, have you ever wanted to select a bunch of columns (by name) in a dataframe, and had to extensively edit the output of [colnames(myData)](https://www.rdocumentation.org/packages/base/topics/colnames) in order to have some tidy code in your script?  `r rdl("hardCode()")` makes it easy to do this:
...
The [stopifnot()](https://www.rdocumentation.org/packages/base/topics/stopifnot) function is super useful for basic sanity checks of functional arguments, ensuring that the arguments supplied by a user have the correct type, length, class, etc.  `r rdl("stopifnotMsg()")` extends [stopifnot()](https://www.rdocumentation.org/packages/base/topics/stopifnot) by making it easy to throw a customized error message for each condition that is checked.
...
`r rdl("parLapplyW()")` parallelizes [lapply()](https://www.rdocumentation.org/packages/base/topics/lapply), and is a wrapper for [parLapply()](https://www.rdocumentation.org/packages/parallel). `r rdl("parLapplyW()")` makes it easy to start the cluster, prepare the workers by exporting variables and/or evaluating expressions, run the [lapply()](https://www.rdocumentation.org/packages/base/topics/lapply) in parallel, and then shut down the cluster---all in one function call.  If the job fails, the cluster is shut down without consequence. Also noteworthy is that `r rdl("parLapplyW()")` works on all the major operating systems:  Linux/Unix, Mac OS, and Windows.
...
- In each R instance, pre-processing or post-processing steps can be performed before and after the call to [lapply()](https://www.rdocumentation.org/packages/base/topics/lapply). These pre-processing and post-processing steps can depend on the instance of R,
...
```
The result is the same as that produced by `r rdl("parLapplyW()")` and [lapply()](https://www.rdocumentation.org/packages/base/topics/lapply):
...
`r rdl("pddply()")` is a wrapper for [plyr::ddply()](https://www.rdocumentation.org/packages/plyr/topics/ddply) that makes it easier to use in parallel and supresses a spurious warning along the way. Here are two workflows that produce identical results using [plyr::ddply()](https://www.rdocumentation.org/packages/plyr/topics/ddply) in parallel and `r rdl("pddply()")`.  First, some preliminaries:
...
```
Here's the workflow with [plyr::ddply](https://www.rdocumentation.org/packages/plyr/topics/ddply):
...
There are circumstances when a continuous version of the discrete binomial cdf can be useful, especially in root finding or optimization. `r rdl("pcbinom()")` is equivalent to [pbinom()](https://www.rdocumentation.org/packages/stats/topics/Binomial) when the first argument is an integer.
...
These functions were designed to operate on a vector of data observed (chronologically) over time.
...
Have you ever been frustrated by the behavior of [filter()](https://www.rdocumentation.org/packages/stats/topics/filter) near the edges of a time series---or miffed by how it handles NA's?  `r rdl("smartFilter()")` gracefully handles the beginning and end of a time series, as well as NA's, by re-normalizing the filter weights as needed.  Let's apply [filter()](https://www.rdocumentation.org/packages/stats/topics/filter) to calculate a moving average consisting of 3 data points:
...
If you need to calculate the cumulative sum of a vector that has missing values, `r rdl("cumsumNA()")` is your friend. Essentially the same as [cumsum()](https://www.rdocumentation.org/packages/base/topics/cumsum), except it doesn't propagate NA's.
...
`r rdl("integ()")` estimates the integral of a real-valued function using Simpson's or the Trapezoid approximation. Why might you want something other than [integrate()](https://www.rdocumentation.org/packages/stats/topics/integrate)?  Because there are occasions when the adaptive quadrature method of [integrate()](https://www.rdocumentation.org/packages/stats/topics/integrate) doesn't behave well---especially if the function is constant over a large portion of the domain. 
...
```
Compare to [pbeta()](https://www.rdocumentation.org/packages/stats/topics/Beta):
```{r integ3, eval = TRUE, echo = TRUE}
pbeta(0.3, 5, 5)
```

## Linear mapping from [a, b] to [c, d]
`r rdl("linearMap()")` maps a numeric vector or scalar from one closed interval to another.  This is especially useful in situations where you want to put multiple variables in the same scale.
