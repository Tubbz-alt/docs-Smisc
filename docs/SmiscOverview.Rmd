...
facilitate my day-to-day work in R.  Most functions operate on their own, each designed for a specific task.

I have organized the documentation into 11 groups of functions that have similar themes.  I provide below a brief synopsis of the motivation for each of the functions in the package, leaving the details to the standard help files, which you can find by following the [bold blue links](rd.html).
...
Then, installation is made easy with [devtools](https://CRAN.R-project.org/package=devtools):
...
I always found it a bit annoying how [load()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/load) places the object(s) in the `.Rdata` file into `.GlobalEnv` without immediately telling you what the name(s) of the object is (are).  `r rdl("loadObject()")` makes it easy to assign the contents of the `.Rdata` file to an object of my choice, like this:
```{r loadObject, eval = FALSE, echo = TRUE}
myName <- loadObject("~/aDir/aFile.Rdata")
```
And thus `myName` contains the contents `aFile.Rdata`.
...
Sometimes it's handy to know whether all the elements in a given row (or column) of a matrix or data frame are missing.  `r rdl("allMissing()")` returns a logical vector indicating whether all the elements in a row (or column) are missing.  It's a nice compliment to [complete.cases()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/complete.cases).
...
Have you ever needed to row bind, [rbind()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/cbind), or column bind, [cbind()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/cbind), or combine/concatenate, [c()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/c), *numerous* objects, usually of the same type?  If you do it iteratively, you'll slow your machine to a crawl:
...
`r rdl("df2list()")` converts a dataframe to a list, in a row-wise fashion (as opposed to [as.list()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/list), which converts in a column-wise fashion):
...
I use `r rdl("list2df()")` probably more than any other `Smisc` function. It converts a list of suitable object types (vectors, dataframes, or lists) into a single dataframe.  It's especially useful for collecting the output from [strsplit()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/strsplit), or the results from [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply).
...
Naively attempting to convert a factor to a numeric vector can easily go wrong.  For example,  [as.numeric()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/as.numeric) probably won't give you what you're looking for. `r rdl("factor2numeric()")` correctly converts a factor to a numeric vector.
...
When writing a script for analysis, have you ever wanted to select a bunch of columns (by name) in a dataframe, and had to extensively edit the output of [colnames(myData)](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/colnames) in order to have some tidy code in your script?  `r rdl("hardCode()")` makes it easy to do this:
...
The [stopifnot()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/stopifnot) function is super useful for basic sanity checks of functional arguments, ensuring that the arguments supplied by a user have the correct type, length, class, etc.  `r rdl("stopifnotMsg()")` extends [stopifnot()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/stopifnot) by making it easy to throw a customized error message for each condition that is checked.
...
`r rdl("parLapplyW()")` parallelizes [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply), and is a wrapper for [parLapply()](https://www.rdocumentation.org/packages/parallel/versions/3.4.1). `r rdl("parLapplyW()")` makes makes it easy to start the cluster, prepare the workers by exporting variables and/or evaluating expressions, run the [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply) in parallel, and then shut down the cluster---all in one function call.  If the job fails, the cluster is shut down without consequence.  
...
`r rdl("plapply()")`, like `r rdl("parLapplyW()")`, is also a parallelization of [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply) for a single workstation. `r rdl("plapply()")` differs from other parallelized implementations of [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply) in that it more intensively reads and writes to disk to manage the parallelization process. It spawns a separate batch instance of R for each parallel job, with each instance producing its own `.Rout` file that can be investigated for errors and warnings. While `r rdl("plapply()")` will be a bit slower than other parallel methods (especially for small jobs) because of the i/o to disk, it contains a number of features that are not readily available in other parallelization functions:
...
- In each R instance, pre-processing or post-processing steps can be performed before and after the call to [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply). These pre-processing and post-processing steps can depend on the instance of R,
...
```
The result is the same as that produced by `r rdl("parLapplyW()")` and [lapply()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/lapply):
...
`r rdl("pddply()")` is a wrapper for [plyr::ddply()](https://www.rdocumentation.org/packages/plyr/versions/1.8.4/topics/ddply) that makes it easier to use in parallel and supresses a spurious warning along the way. Here are two workflows that produce identical results using [plyr::ddply()](https://www.rdocumentation.org/packages/plyr/versions/1.8.4/topics/ddply) in parallel and `r rdl("pddply()")`.  First, some preliminaries:
...
```
Here's the workflow with [plyr::ddply](https://www.rdocumentation.org/packages/plyr/versions/1.8.4/topics/ddply):
...
There are circumstances when a continuous version of the discrete binomial cdf can be useful, especially in root finding or optimization. `r rdl("pcbinom()")` is equivalent to [pbinom()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/Binomial) when the first argument is an integer.
...
Have you ever been frustrated by the behavior of [filter()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/filter) near the edges of a time series---or miffed by how it handles NA's?  `r rdl("smartFilter()")` gracefully handles the beginning and end of a time series, as well as NA's, by re-normalizing the filter weights as needed.  Let's apply [filter()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/filter) to calculate a moving average consisting of 3 data points:
...
`r rdl("cusum()")` will calculate the Cusum statistics for a cumulative sum chart of the form $C[i] = max(0, C[i-1] + X[i] - k)$ that signals when $C[i] > h$.  Here's a simple example:
```{r cusumset, eval = TRUE, echo = FALSE}
set.seed(15)
```
```{r cusum, eval = TRUE, echo = TRUE}
# Calculate the Cusum
y <- cusum(rnorm(25), k = 0.2, h = 2)

# Plot results
plot(y, main = "Upper Cusum chart of a standard normal variate", las = 1)
```
...
`r rdl("comboList()")` produces a list that contains all possible combinations of a set of `n` objects indexed by `1:n`.  As this can be computationally expensive, it can be easily parallelized with `njobs > 1`. This extends [combn()](https://www.rdocumentation.org/packages/utils/versions/3.4.1/topics/combn), which produces all the possible combinations of a specific size.  For example, here are all the possible combinations of 3 items, taken 1, 2, and 3 at a time:
...
If you need to calculate the cumulative sum of a vector that has missing values, `r rdl("cumsumNA()")` is your friend. Essentially the same as [cumsum()](https://www.rdocumentation.org/packages/base/versions/3.4.1/topics/cumsum), except it doesn't propagate NA's.
...
`r rdl("integ()")` estimates the integral of a real-valued function using Simpson's or the Trapezoid approximation. Why might you want something other than [integrate()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/integrate)?  Because there are occasions when the adaptive quadrature method of [integrate()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/integrate) doesn't behave well---especially if the function is constant over a large portion of the domain. 
...
```
Compare to [pbeta()](https://www.rdocumentation.org/packages/stats/versions/3.4.1/topics/Beta):
