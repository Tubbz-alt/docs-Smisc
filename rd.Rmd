...
<p><strong>Version:</strong> 0.3.8</p>
...
supply them so that all cluster nodes have the correct environment set up for computing.</dd>
  ...
  <p class="rd-p">An innocuous warning is thrown when <code><a href=http://www.rdocumentation.org/packages/plyr/topics/ddply>plyr::ddply</a></code> is called in parallel:
...
<code>D[2]</code>.</dd>
  ...
  <p class="rd-p">The mapping is <code class = eq>f : D \rightarrow R</code>, where <code class = eq>f(D[1]) = R[1]</code> and
