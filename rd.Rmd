...
<dl class="rd-dl">
  ...
  <p class="rd-p">The object(s) contained in <code>RdataFiles</code>, organized into lists and named as
required to distinguish them completely.  See Examples.</p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/load>load</a></code>
...
"packageName::dataSetName".</dd>
  ...
  <p class="rd-p">A data frame (or list of data frames) containing the requested
data.</p>
...
<code><a href=http://www.rdocumentation.org/packages/utils/topics/data>data</a></code>,<code><a href=#loadobject>loadObject</a></code>,<code><a href=http://www.rdocumentation.org/packages/utils/topics/read.table>read.csv</a></code>
...
<code>= FALSE</code> identifies entire missing columns</dd>
  ...
  <p class="rd-p">A logical vector that is true if all the elements in the
corresponding row (or column) are NAs.</p>
...
<code><a href=http://www.rdocumentation.org/packages/stats/topics/complete.cases>complete.cases</a></code>
...
<h4>Usage</h4>
<pre class="r"><code>dframeEquiv(d1, d2, maxAbsError = 1e-12, maxRelError = 1e-14, verbose = TRUE)</code></pre>
...
and then compared directly.</p>
  ...
  <p class="rd-p">Invisibly returns a list with the following components.  (If the
...
<code>NULL</code>).  equiv<code>=TRUE</code> if <code>d1</code> is equivalent to
<code>d2</code> msgMessages that describe the comparison.  (These are
printed when <code>verbose=TRUE</code>.) frac.equivThe fraction of
matrix elements that are equivalent loc.inequivA data frame
indicating the row and column coordinate locations of the elements that are
not equivalent eqiv.matrixA boolean matrix with the same dimension
as <code>d1</code> and <code>d2</code>, indicating the equivalent elements</p>


  <h4>References</h4>

  <p class="rd-p"><a href = https://randomascii.wordpress.com/category/floating-point/>https://randomascii.wordpress.com/category/floating-point/</a></p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/all.equal>all.equal</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/identical>identical</a></code>
...
<p class="rd-p">A wrapper for <code><a href='http://www.rdocumentation.org/packages/base/topics/cbind'>rbind</a></code> and <code><a href='http://www.rdocumentation.org/packages/base/topics/cbind'>cbind</a></code> to quickly bind
numerous objects together at once using a single call to <code><a href='http://www.rdocumentation.org/packages/base/topics/cbind'>rbind</a></code> or
 <code><a href='http://www.rdocumentation.org/packages/base/topics/cbind'>cbind</a></code>. This function is most helpful when there are
...
<dl class="rd-dl">
  ...
  <dt>type</dt>
  <dd class="rd-dd">The type of binding, "row" for <code><a href=http://www.rdocumentation.org/packages/base/topics/cbind>rbind</a></code> or "col" for
<code><a href=http://www.rdocumentation.org/packages/base/topics/cbind>cbind</a></code>, and "c" for concatenating using <code><a href=http://www.rdocumentation.org/packages/base/topics/c>c</a></code>.  Defaults to
"row".</dd>
</dl>

  <h4>Value</h4>

  <p class="rd-p">The bound object</p>
...
elements which occur when not all the matrices have the same column names.</dd>
  ...
  <p class="rd-p">Produces a matrix with a union of the column names.  Empty elements
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/cbind>rbind</a></code>
...
In this case, a data frame or matrix with either 0 columns or 0 rows is returned.</p>
  ...
  <p class="rd-p">The matrix or data frame is returned with the selected columns or rows.</p>
...
be the same type.  Defaults to list.</dd>
  ...
  <p class="rd-p">A list where each element consists of a named list, single-row data
...
<code><a href=#list2df>list2df</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/list>as.list</a></code>
...
via <code><a href=http://www.rdocumentation.org/packages/base/topics/as.data.frame>as.data.frame</a></code> and the resulting data frames must have the
...
etc.</p>
  ...
  <p class="rd-p">If <code>vList</code> is list of data frames, a data frame resulting from
...
contains the second elements of the list vectors, etc.</p>
...
are named o1, o2, o3, etc.</p>
  ...
  <p class="rd-p">Invisibly returns a character vector of the names of the objects that were created, and assigns the objects to
the environment specified by <code>envir</code></p>
...
<dl class="rd-dl">
  ...
  <p class="rd-p">The same dataframe with the factor variables converted to character
variables.</p>
...
<dl class="rd-dl">
  ...
  <p class="rd-p"><code>x</code> converted to a numeric vector</p>
...
<dl class="rd-dl">
  ...
  <p class="rd-p">If <code>as.numeric(x)</code> produces an error or warning, <code>x</code> is
returned unchanged.  Otherwise, <code>as.numeric(x)</code> is returned.</p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/numeric>as.numeric</a></code>
...
exist (see examples below), a <code>""</code> is returned.</p>
  ...
  <p class="rd-p">Character vector of filename extensions</p>
...
<code><a href=#getpath>getPath</a></code>, <code><a href=#grablast>grabLast</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/basename>basename</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/basename>dirname</a></code>
...
exist (see examples below), a <code>""</code> is returned.</p>
  ...
  <p class="rd-p">Character vector with pathnames only, the filename removed</p>
...
<code><a href=#strippath>stripPath</a></code>, <code><a href=#grablast>grabLast</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/basename>basename</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/basename>dirname</a></code>
...
exist (see examples below), a <code>""</code> is returned.</p>
  ...
  <p class="rd-p">Character vector with the last "." and the
...
exist (see examples below), a <code>""</code> is returned.</p>
  ...
  <p class="rd-p">Character vector with leading path removed from the filenames</p>
...
<dl class="rd-dl">
  ...
  <p class="rd-p">Character vector of the strings that appear after the last instance of <code>split.char</code></p>
...
Additional functions for filename manipulations:  <code><a href=#stripextension>stripExtension</a></code>, <code><a href=#getextension>getExtension</a></code>,
<code><a href=#strippath>stripPath</a></code>, <code><a href=#getpath>getPath</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/basename>basename</a></code>, <code><a href=http://www.rdocumentation.org/packages/base/topics/basename>dirname</a></code>
...
(excluding the period)</dd>
  ...
  <p class="rd-p">Character string of the form <code>description_YYYY-MM-DD_HHMMSS.extension</code></p>
...
<code>vec</code> may be character when <code>side = left</code>.</p>
  ...
  <p class="rd-p">Character vector with the leading (or trailing) elements padded
with zeros.</p>
...
(<code>FALSE</code>)</dd>
  <dt>...</dt>
  <dd class="rd-dd">Additional arguments to <code><a href=http://www.rdocumentation.org/packages/base/topics/cat>cat</a></code></dd>
</dl>

  <h4>Value</h4>

  <p class="rd-p">Prints code (via <code><a href=http://www.rdocumentation.org/packages/base/topics/cat>cat</a></code>) that will create the vector.  This code
can then be copied into other source code.  Returns nothing.</p>
...
<h3>Display the contents of a text file to the R console</h3>
...
to the <code>n</code> argument of <code><a href=http://www.rdocumentation.org/packages/base/topics/readLines>readLines</a></code>. The default is <code>-1</code>,
...
the last 6 lines that were read.</dd>
  ...
  <p class="rd-p">Returns nothing, but it does display the contents of the file on the R console.</p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/readLines>readLines</a></code>
...
is passed to the <code>digits</code> argument of <code><a href=http://www.rdocumentation.org/packages/base/topics/Round>round</a></code>.</dd>
  <dt>abbrev</dt>
...
but the results wont be easily readable.</p>
  ...
  <p class="rd-p">Invisibly returns a character string containing the names of the
...
if the <code>elements</code> for selection are specified incorrectly.</p>
  ...
  <p class="rd-p">A character vector with elements that were selected from <code>cVec</code> using <code>elements</code></p>
...
<code>FALSE</code>, <code>sourceDir</code> will stop if a file sources with an error.</dd>
  <dt>...</dt>
  <dd class="rd-dd">Additional arguments to <code><a href=http://www.rdocumentation.org/packages/base/topics/source>source</a></code></dd>
</dl>

  <h4>Details</h4>

  <p class="rd-p">In addition to sourcing files for general use, this function is also useful in package development to
verify there are no syntax errors prior to building and compilation.</p>


  <h4>Value</h4>

  <p class="rd-p">Invisibly returns a character vector containing the files that were
...
<p class="rd-p">A more flexible version of <code><a href='http://www.rdocumentation.org/packages/base/topics/stopifnot'>stopifnot</a></code> that allows you to control the error message returned by
...
See examples.</dd>
  ...
  <p class="rd-p">A call to <code><a href=http://www.rdocumentation.org/packages/base/topics/stop>stop</a></code> with the error messages from each condition that was <code>FALSE</code>.  If all
conditions are <code>TRUE</code>, returns <code>NULL</code>.</p>
...
<dl class="rd-dl">
  <dt>expr</dt>
  <dd class="rd-dd">Any R <code><a href=http://www.rdocumentation.org/packages/base/topics/expression>expression</a></code>.</dd>
  <dt>units</dt>
...
< 2 hours, use minutes. Else if < 2 days, use hours.  Otherwise, use days.</p>
  ...
  <p class="rd-p">If <code>return.time = FALSE</code>, invisibly returns the evaluation of
<code>expr</code>.  If <code>return.time = TRUE</code>, invisibly returns a list with
the following components:
outThe evaluation of <code>expr</code>
elapsedThe elapsed time to evaluate <code>expr</code>
unitsThe time units of the elapsed time</p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/proc.time>proc.time</a></code>
...
<p class="rd-p">A wrapper to make calls to <code><a href='http://www.rdocumentation.org/packages/parallel/topics/clusterApply'>parLapply</a></code> easier by initializing the cluster, exporting objects and expressions to the
worker nodes, and shutting down the cluster.</p>

<h4>Usage</h4>
<pre class="r"><code>parLapplyW(
  X,
  FUN,
  ...,
...
<dl class="rd-dl">
  ...
  <dt>expr</dt>
  <dd class="rd-dd">An expression that will be evaluated on each worker node via a call to <code><a href=http://www.rdocumentation.org/packages/parallel/topics/clusterApply>clusterEvalQ</a></code></dd>
  <dt>varlist</dt>
  <dd class="rd-dd">Character vector of names of objects to export to each worker node via <code><a href=http://www.rdocumentation.org/packages/parallel/topics/clusterApply>clusterExport</a></code></dd>
  <dt>envir</dt>
  ...
  <p class="rd-p">The same result given by <code>lapply(X, FUN, ...)</code></p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/lapply>lapply</a></code>, <code><a href=http://www.rdocumentation.org/packages/parallel/topics/clusterApply>parLapply</a></code>, <code><a href=#plapply>plapply</a></code>
...
<p class="rd-p">Parses a large list into subsets and submits a separate batch R job that calls <code><a href='http://www.rdocumentation.org/packages/base/topics/lapply'>lapply</a></code>
on the  subset.  <code>plapply</code> has some features that may not be readily available in
other parallelization functions like <code><a href='http://www.rdocumentation.org/packages/parallel/topics/mclapply'>mclapply</a></code> and <code><a href='http://www.rdocumentation.org/packages/parallel/topics/clusterApply'>parLapply</a></code>:
<ul>
...
before and after the call to <code><a href='http://www.rdocumentation.org/packages/base/topics/lapply'>lapply</a></code></li>
</ul>
...
loaded in each new instance of R, using <code><a href=http://www.rdocumentation.org/packages/base/topics/library>library</a></code>.</dd>
  <dt>header.file</dt>
  <dd class="rd-dd">Text string indicating a file that will be initially
sourced prior calling <code><a href=http://www.rdocumentation.org/packages/base/topics/lapply>lapply</a></code> in order to create an
...
and then applies <code>FUN</code> to each sublist using <code><a href=http://www.rdocumentation.org/packages/base/topics/lapply>lapply</a></code>.</p>
...
<code>needed.objects</code></p>
  ...
  <p class="rd-p"></li>
<li> <code><a href=http://www.rdocumentation.org/packages/base/topics/lapply>lapply</a></code> is called on the sublist, the sublist is called
<code>X.i</code></p>
...
call is made:  <code>lapply(X, FUN, ...)</code></p>
  ...
  <p class="rd-p">A list equivalent to that returned by <code>lapply(X, FUN, ...)</code>.</p>
...
<code><a href=#parlapplyw>parLapplyW</a></code>, <code><a href=#dfplapply>dfplapply</a></code>, <code><a href=http://www.rdocumentation.org/packages/parallel/topics/clusterApply>parLapply</a></code>, <code><a href=http://www.rdocumentation.org/packages/parallel/topics/mclapply>mclapply</a></code>
...
jobs.</dd>
  ...
  <p class="rd-p">A list or data frame containing the results of processing each row
of <code>X</code> with <code>FUN</code>.</p>
...
<p class="rd-p">Parallel implementation of <code><a href='http://www.rdocumentation.org/packages/plyr/topics/ddply'>plyr::ddply</a></code> that suppresses a spurious warning when
<code><a href='http://www.rdocumentation.org/packages/plyr/topics/ddply'>plyr::ddply</a></code> is called in parallel.
All of the arguments except <code>njobs</code> are passed directly to arguments of the same name in
<code><a href='http://www.rdocumentation.org/packages/plyr/topics/ddply'>plyr::ddply</a></code>.</p>

<h4>Usage</h4>
<pre class="r"><code>pddply(
  .data,
  .variables,
  .fun = NULL,
  ...,
...
<dl class="rd-dl">
  ...
  <dt>.progress</dt>
  <dd class="rd-dd">name of the progress bar to use, see <code><a href=http://www.rdocumentation.org/packages/plyr/topics/create_progress_bar>plyr::create_progress_bar</a></code></dd>
  <dt>.inform</dt>
...
preserved (FALSE) or dropped (TRUE, default)</dd>
  <dt>.paropts</dt>
  <dd class="rd-dd">a list of additional options passed into the <code><a href=http://www.rdocumentation.org/packages/foreach/topics/foreach>foreach::foreach</a></code> function
...
supply them so that all cluster nodes have the correct environment set up for computing.</dd>
  ...
  <p class="rd-p">An innocuous warning is thrown when <code><a href=http://www.rdocumentation.org/packages/plyr/topics/ddply>plyr::ddply</a></code> is called in parallel.  This function catches and hides that warning, which looks like this:
<code></p>
...
1: <anonymous>: ... may be used in an incorrect context: .fun(piece, ...)
  ...
  <p class="rd-p">If <code>njobs = 1</code>, a call to <code><a href=http://www.rdocumentation.org/packages/plyr/topics/ddply>plyr::ddply</a></code> is made without parallelization, and anything
supplied to <code>.paropts</code> is ignored. See the documentation for <code><a href=http://www.rdocumentation.org/packages/plyr/topics/ddply>plyr::ddply</a></code> for additional details.</p>


  <h4>Value</h4>

  <p class="rd-p">The object data frame returned by <code><a href=http://www.rdocumentation.org/packages/plyr/topics/ddply>plyr::ddply</a></code></p>
...
<code><a href=http://www.rdocumentation.org/packages/plyr/topics/ddply>plyr::ddply</a></code>
...
<h4>Usage</h4>
<pre class="r"><code>doCallParallel(
  fun,
  x,
  ...,
  njobs = parallel::detectCores() - 1,
...
no randomization is performed.</dd>
  ...
  <p class="rd-p">This function is a parallelized wrapper for <code><a href=http://www.rdocumentation.org/packages/base/topics/do.call>do.call</a></code> designed for the case where <code>fun</code> is
...
to <code>c(fun(x1), fun(x2))</code> in order for <code>doCallParallel</code> to work properly.</p>
  ...
  <p class="rd-p">The same result that would be had by calling <code>fun(x, ...)</code>, except calculated in parallel</p>
...
<h4>Usage</h4>
<pre class="r"><code>parseJob(n, njobs, collate = FALSE, random.seed = NULL, text.to.eval = FALSE)</code></pre>
...
This is especially useful when <code>n</code> is very large. (See <code>Value</code> section below).</dd>
  ...
  <p class="rd-p">When <code>text.to.eval = FALSE</code>,
  ...
  numbers corresponding to the group.</p>
...
<code><a href='http://www.rdocumentation.org/packages/grDevices/topics/postscript'>postscript</a></code>, <code><a href='http://www.rdocumentation.org/packages/grDevices/topics/pdf'>pdf</a></code>, <code><a href='http://www.rdocumentation.org/packages/grDevices/topics/png'>jpeg</a></code>,
<code><a href='http://www.rdocumentation.org/packages/grDevices/topics/png'>tiff</a></code>, <code><a href='http://www.rdocumentation.org/packages/grDevices/topics/png'>png</a></code>, or <code><a href='http://www.rdocumentation.org/packages/grDevices/topics/png'>bmp</a></code>.</p>
...
that do not match are silently ignored.</dd>
  ...
  <p class="rd-p">The graphics device is opened and the filename is invisibly
returned.</p>
...
and plotted, just as in <code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code> in the <span class = "pkg">graphics</span> package.</dd>
  <dt>col</dt>
  <dd class="rd-dd">A vector of colors to use in the plotting. Its length should match the length of <code>fun</code>. See <code><a href=http://www.rdocumentation.org/packages/graphics/topics/par>par</a></code> for
more info about the <code>col</code> graphics parameter.</dd>
  <dt>lty</dt>
  <dd class="rd-dd">A vector of line types to use in the plotting. Its length should match the length of <code>fun</code>. See <code><a href=http://www.rdocumentation.org/packages/graphics/topics/par>par</a></code> for
...
<code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code>.</dd>
  <dt>legendLabels</dt>
  <dd class="rd-dd">A character vector with descriptive names that will appear in the legend, corresponding to each function
If <code>NULL</code>, no legend is drawn. This character vector is passed to the <code>legend</code> argument in <code><a href=http://www.rdocumentation.org/packages/graphics/topics/legend>legend</a></code>
from the <span class = "pkg">graphics</span> package.</dd>
  ...
  <dt>...</dt>
  <dd class="rd-dd">Additional graphical arguments passed to <code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code>, <code><a href=http://www.rdocumentation.org/packages/graphics/topics/lines>lines</a></code>, and
<code><a href=http://www.rdocumentation.org/packages/graphics/topics/legend>legend</a></code>.  If an argument name specified in <code>...</code> matches an argument name in any
of these three functions, the argument is passed to that function.
For example, the line width, <code>lwd</code>, would be passed to all three (<code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code>, <code><a href=http://www.rdocumentation.org/packages/graphics/topics/lines>lines</a></code>, and
<code><a href=http://www.rdocumentation.org/packages/graphics/topics/legend>legend</a></code>).</dd>
</dl>

  <h4>Value</h4>

  <p class="rd-p">The plot of the function(s)</p>
...
<h4>Usage</h4>
<pre class="r"><code>smartTimeAxis(
  time.vec,
  nticks = 15,
  side = 1,
...
presumed to be ordered chronologically</dd>
  ...
  <dt>side</dt>
  <dd class="rd-dd">Same as the <code>side</code> argument in <code><a href=http://www.rdocumentation.org/packages/graphics/topics/axis>axis</a></code></dd>
  <dt>time.format</dt>
...
15 seconds, etc., or 1, 2, 5, 10, minutes etc., or 0.5, 1, 1.5 hours, etc.</p>
  ...
  <p class="rd-p">Places the axis on the plot.</p>
...
<code><a href=http://www.rdocumentation.org/packages/graphics/topics/axis.POSIXct>axis.POSIXct</a></code>
...
single value.</dd>
  <dt>...</dt>
  <dd class="rd-dd">additional arguments to <code><a href=http://www.rdocumentation.org/packages/graphics/topics/lines>lines</a></code>.</dd>
</dl>
...
different vertical lengths, widths, and <code>blankMiddle</code> values if desired.</p>
  ...
  <p class="rd-p">Nothing is returned, the error bar(s) is/are drawn on the plot</p>
...
binomials are used to calculate the mass or the distribution functions.</p>
  ...
  <p class="rd-p"><code>dkbinom</code> gives the mass function, <code>pkbinom</code> gives the
...
more quickly.</p>
  ...
  <p class="rd-p">The Butler method utilizes the exact algorithm discussed by: 
...
Statistics, Stanford University. <a href = https://apps.dtic.mil/dtic/tr/fulltext/u2/a266969.pdf>https://apps.dtic.mil/dtic/tr/fulltext/u2/a266969.pdf</a></p>
...
<code><a href=http://www.rdocumentation.org/packages/stats/topics/Binomial>dbinom</a></code>, <code><a href=http://www.rdocumentation.org/packages/stats/topics/Binomial>pbinom</a></code>
...
as <code>log(p)</code>.</dd>
  ...
  <p class="rd-p"><code>pcbinom</code> is equivalent to <code><a href=http://www.rdocumentation.org/packages/stats/topics/Binomial>pbinom</a></code> for integer values of
...
other arguments have length 1.  See examples below.</p>
  ...
  <p class="rd-p">Returns a continuous version of the binomial distribution function.</p>
...
<code><a href=http://www.rdocumentation.org/packages/stats/topics/Binomial>pbinom</a></code>, <code><a href=http://www.rdocumentation.org/packages/stats/topics/Beta>pbeta</a></code>
...
<code><a href=#plotfun>plotFun</a></code>, <code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code>, or <code><a href=http://www.rdocumentation.org/packages/graphics/topics/abline>abline</a></code></dd>
</dl>
...
the <code>cdf</code> is not provided, in which case <code>pdf</code> is integrated, and/or 3) when <code>checkUnimodal</code> is large.</p>
  ...
  <p class="rd-p">A list of class <code>hpd</code> that contains the following elements:
<dl>
<dt>lower</dt><dd>The lower endpoint of the highest posterior density interval</dd>
<dt>upper</dt><dd>The lower endpoint of the highest posterior density interval</dd>
<dt>prob</dt><dd>The acheived probability of the interval</dd>
<dt>cut</dt><dd>The horizontal cut point that gave rise to the interval</dd>
<dt>mode</dt><dd>The mode of the density</dd>
<dt>pdf</dt><dd>The probability density function</dd>
<dt>support</dt><dd>The support of the pdf</dd>
</dl></p>
...
current term in the series is less than <code>tol</code>.</dd>
  ...
  <p class="rd-p">Returns a named vector with the following components muThe
UMVUE of the mean se.muThe UMVUE standard error of the mean
sigmaThe UMVUE of the standard deviation</p>
...
hour time)</p>
  ...
  <p class="rd-p">A list with these components: dateA character vector of the
form requested by <code>date.outformat</code>. timeA character vector of
the form requested by <code>time.outformat</code> or an empty character vector of
the form "" if the time is not present in the input vector <code>dt</code>.
dtA character vector containing the combined datetime using the
requested formats.  If time is not present in the input vector <code>dt</code>,
then simply the date is returned. date.posixA vector of class
"POSIXt POSIXct" containing the date.  This is only returned if
<code>posix = TRUE</code>. dt.posixA vector of class "POSIXt POSIXct"
containing the datetime.  This is only returned if <code>posix = TRUE</code> and
time values are present in the argument <code>dt</code>. weekdayA
character vector indicating the days of the week.  This is only returned if
<code>weekday = TRUE</code>.</p>
...
<h4>Usage</h4>
<pre class="r"><code>smartFilter(y, weights, min.window = 1, start = 1, skip = 1, balance = TRUE)</code></pre>
...
non-missing value occur on or after the center point of the window.</dd>
  ...
  <p class="rd-p"><code>smartFilter</code> has very similar behavior to <code><a href=http://www.rdocumentation.org/packages/stats/topics/filter>filter</a></code>,
...
returned for the corresponding index.</p>
  ...
  <p class="rd-p">Returns the moving dot product</p>
...
<code><a href=#movavg2>movAvg2</a></code>, <code><a href=http://www.rdocumentation.org/packages/stats/topics/filter>filter</a></code>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/print.default>print.default</a></code> and <code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code>, respectively.</dd>
  <dt>x</dt>
...
weights as possible.</p>
  ...
  <p class="rd-p">An object class <code>movAvg2</code>, which is a numeric vector containing the moving average (dot product) of the
series, with attributes that describe the weights.  If <code>y = NULL</code>, <code>numeric(0)</code> is returned.</p>
...
<code><a href=#smartfilter>smartFilter</a></code>, <code><a href=http://www.rdocumentation.org/packages/stats/topics/filter>filter</a></code>
...
<dl class="rd-dl">
  ...
  <dt>...</dt>
  <dd class="rd-dd">Additional arguments to <code><a href=http://www.rdocumentation.org/packages/base/topics/print.default>print.default</a></code> or <code><a href=http://www.rdocumentation.org/packages/graphics/topics/plot.default>plot.default</a></code>.  Ignored by the <code>signal</code> method.</dd>
  <dt>indexes</dt>
...
given by the log-likelihood ratio, in which case <code>k = 0</code> would make sense.</p>
  ...
  <p class="rd-p">A object of class <code>cusum</code>, which is a vector of the Cusum statistics, along with the following attributes:
...
has the same timestamps as <code>v1</code>.</p>
  ...
  <p class="rd-p">if <code>full = FALSE</code> then the difference (after matching) of
<code>(v1 - v2)</code> is returned. Otherwise, a data frame is returned that
shows how the vectors were matched and the resulting difference vector.</p>
...
<h4>Usage</h4>
<pre class="r"><code>timeIntegration(
  data,
  time = names(data),
  lower = time[1],
...
predict the resulting data value.</p>
  ...
  <p class="rd-p">The approximation of the integral by joining the points in the
series in a linear fashion and calculating the area under this "curve".</p>
...
calculating the combinations, using <code><a href=#parlapplyw>parLapplyW</a></code></dd>
</dl>

  <h4>Details</h4>

  <p class="rd-p">Uses <code><a href=http://www.rdocumentation.org/packages/utils/topics/combn>combn</a></code> to identify the combinations.</p>


  <h4>Value</h4>

  <p class="rd-p">A list of class <code>combolist</code> is invisibly returned with the two
components shown below.  If <code>outFile</code> is not <code>NULL</code>, this same
list is saved to <code>outFile</code>: lenThe total number of
combinations pListA list where each element contains an integer
representation of one combination of the predictors</p>
...
<dl class="rd-dl">
  ...
  <p class="rd-p">In the sequence <em>x[1], x[2], ..., x[n]</em>, <code>cumMax</code>
returns the vector <em>y</em> such that for each <em>i = 1,...,n</em>,
<em>y[i] = max(x[j]; j = 1,...,i)</em></p>
...
to be <code>NA</code>, but the <code>NA</code> will not be propagated.</p>
  ...
  <p class="rd-p">The vector of cumulative sums.</p>
...
<code><a href=http://www.rdocumentation.org/packages/base/topics/cumsum>cumsum</a></code>
...
the way <code><a href=http://www.rdocumentation.org/packages/base/topics/duplicated>duplicated</a></code> identifies duplicates. As such, the first
...
produced by <code><a href=http://www.rdocumentation.org/packages/base/topics/qr>qr</a></code>, and if not equal, a warning is issued.  The
value of <code>tol</code> is passed to <code><a href=http://www.rdocumentation.org/packages/base/topics/qr>qr</a></code>, but the criteria of
convergence for <code><a href=http://www.rdocumentation.org/packages/base/topics/qr>qr</a></code> is assuredly different from that used here
...
<code>findDepMat</code> is likely to be slow for large matrices.</p>
  ...
  <p class="rd-p">A logical vector, equal in length to the number of rows (columns)
of <code>X</code>, with <code>TRUE</code> indicating that the row (column) is linearly
dependent on a previous row (column).</p>
...
<h4>Usage</h4>
<pre class="r"><code>integ(y, x = NULL, a = NULL, b = NULL, method = c("simpson", "trapezoid"))</code></pre>
...
odd or even. The elements of <code>x</code> may be irregularly spaced.</p>
  ...
  <p class="rd-p">A single numeric estimate of the integral of f(x) over [a, b]
(Simpson) or over the range of <code>x</code> (trapezoid).</p>
...
<code><a href=http://www.rdocumentation.org/packages/stats/topics/integrate>integrate</a></code>
...
<p class="rd-p">Linear mapping of a numeric vector or scalar from one closed interval
to another</p>
...
<code>D[2]</code>.</dd>
  ...
  <p class="rd-p">The mapping is <code class = eq>f : D --> R</code>, where <code class = eq>f(D[1]) = R[1]</code> and
<code class = eq>f(D[2]) = R[2]</code>.</p>


  <h4>Value</h4>

  <p class="rd-p">The linear mapping of <code>x</code> from <code>D</code> to <code>R</code></p>
...
<dl>
<dt>time</dt><dd>a POSIXt time variable</dd>
<dt>x</dt><dd>a numeric vector</dd>
</dl></p>
