...
<h4>Description</h4>
<p>Sego Miscellaneous: A collection of functions for statistical
computing and data manipulation in R.</p>
...
<dl class="rd-dl">
  <dt>RdataFiles</dt>
  <dd class="rd-dd">A character vector containing the .Rdata filenames</dd>
</dl>
...
<h3>Select rows or columns from data frames or matrices</h3>

<p class="rd-p">Select rows or columns from data frames or matrices while always returning a data frame or a matrix</p>
...
<h3>Sort a dataframe</h3>

<p class="rd-p">Sort a dataframe by one or more ascending or descening variables</p>

<h4>Usage</h4>
<pre class="r"><code>sortDF(form, dat)</code></pre>

<h4>Arguments</h4>
<dl class="rd-dl">
  <dt>form</dt>
  <dd class="rd-dd">A formula with the variable names to use for sorting</dd>
  <dt>dat</dt>
  <dd class="rd-dd">The dataframe to sort</dd>
</dl>

  <h4>Details</h4>

  <p class="rd-p">The order of the arguments <code>form</code> and <code>dat</code> is
interchangeable.  In the formula, use + for ascending, - for
decending. Sorting is left to right in the formula. 
For example, to sort the dataframe <code>Oats</code> with sorting variables <code>Block</code> and
<code>Variety</code>, useage is either of the following: 
<code>sortDF(~ Block - Variety, Oats)</code>  or
<code>sortDF(Oats, ~ - Variety + Block)</code></p>
...
The sorted dataframe
  ...
  <p class="rd-p"><a href = #https://stat.ethz.ch/pipermail/r-help/2004-september/057954>https://stat.ethz.ch/pipermail/r-help/2004-September/057954.html</a></p>



<h4>Examples</h4>
<pre class="r"><code>d <- data.frame(b = factor(c("Hi","Med","Hi","Low"), levels = c("Low","Med","Hi"), 
                           ordered = TRUE),
                x = c("A", "D", "A", "C"),
                y = c(8, 3, 9, 9),
                z = c(1, 1, 1, 2))
...
<h3>Silent wrapper for coercing a vector to numeric</h3>
...
<p class="rd-p">Get the extension of a vector of filenames, assuming that the extension is the set of characters that follows the last
<code>.</code>.  A wrapper for <code><a href='grabLast.html'>grabLast</a></code>.</p>
...
<p class="rd-p">Remove the extension of a vector of filenames, assuming that the extension is the set of characters that follows the last
"."</p>
...
<h3>Imbeds the present datetime into a file name</h3>
...
<dl class="rd-dl">
  <dt>description</dt>
  <dd class="rd-dd">Character string giving the base name of the file</dd>
  <dt>extension</dt>
  <dd class="rd-dd">Character string giving the extension of the file
...
<dl class="rd-dl">
  ...
  <dt>num</dt>
  <dd class="rd-dd">The number of zeros that will be padded. If <code>NULL</code>, the
...
<dl class="rd-dl">
  <dt>x</dt>
  <dd class="rd-dd">A vector (numeric, character, or logical)</dd>
  <dt>vname</dt>
...
<h3>Display the contents of a file to the R terminal</h3>

<p class="rd-p">Display the contents of a file to the R terminal, like the <code>more</code> command in Unix</p>

<h4>Usage</h4>
<pre class="r"><code>more(file, n = -1L, display = c("all", "head", "tail"))</code></pre>
...
to the <code>n</code> argument of <code><a href=http://www.inside-r.org/r-doc/base/readLines>readLines</a></code>. The default is <code>as.integer(-1)</code>,
...
<h4>Usage</h4>
<pre class="r"><code>stopifnotMsg(...)</code></pre>
...
the following components:</p>
  ...
  <p class="rd-p"><dt>out</dt><dd>The evaluation of <code>expr</code></dd>
 <dt>elapsed</dt><dd>The elapsed time
to evaluate <code>expr</code></dd>
 <dt>units</dt><dd>The time units of the elapsed time</dd></p>
...
(by submitting multiple batch R jobs).  It is a convenient wrapper for <code>plapply</code>, modified
...
<h3>Two-way Interaction Plot with Error Bar</h3>

<p class="rd-p">Plots the mean (or other summary) of the response for two-way combinations
of factors, thereby illustrating possible interactions.
This modifies <code><a href='http://www.inside-r.org/r-doc/stats/interaction.plot'>interaction.plot</a></code> by adding the <code>errorBar</code> and
<code>jitterErrorBars</code> arguments, making it possible to add error bars of the
same length to the plot.</p>

<h4>Usage</h4>
<pre class="r"><code>interactionPlot(x.factor, trace.factor, response, fun = mean,
  errorBar = NULL, jitterErrorBars = NULL, type = c("l", "p", "b", "o",
  "c"), legend = TRUE, trace.label = deparse(substitute(trace.factor)),
  ...
  xtick = FALSE, xaxt = par("xaxt"), axes = TRUE, ...)</code></pre>

<h4>Arguments</h4>
<dl class="rd-dl">
  <dt>x.factor</dt>
  <dd class="rd-dd">a factor whose levels will form the x axis.</dd>
  <dt>trace.factor</dt>
  <dd class="rd-dd">another factor whose levels will form the traces.</dd>
  <dt>response</dt>
  <dd class="rd-dd">a numeric variable giving the response.</dd>
  <dt>fun</dt>
  <dd class="rd-dd">the function to compute the summary. Should return a single real
value.</dd>
  <dt>errorBar</dt>
  <dd class="rd-dd">A list with required elements <code>barLength</code> and
<code>width</code>, and optional element <code>blankMiddle</code>. These correspond to
the arguments of <code><a href=#verterrorbar>vertErrorBar</a></code>.  Defaults to <code>NULL</code>, in
which case no error bars are drawn.  Currently, error bars must all be the same
length and have the same width. Thus, elements in the list should be atomic (of length 1).</dd>
  <dt>jitterErrorBars</dt>
  <dd class="rd-dd">A list with the <code>factor</code> and/or <code>amount</code> arguments that will be
passed to <code><a href=http://www.inside-r.org/r-doc/base/jitter>jitter</a></code>, indicating the amount (and/or factor) by which the x-values of the points
(and the error bars) should be jittered for easier readibility. No jittering is performed if
<code>jitterErrorBars = NULL</code>. This argument is ignored if <code>errorBar = NULL</code>.</dd>
  <dt>type</dt>
  <dd class="rd-dd">the type of plot (see <code><a href=http://www.inside-r.org/r-doc/graphics/plot.default>plot.default</a></code>): lines or
points or both.</dd>
  <dt>legend</dt>
  <dd class="rd-dd">logical. Should a legend be included?</dd>
  <dt>trace.label</dt>
  <dd class="rd-dd">overall label for the legend.</dd>
  <dt>fixed</dt>
  <dd class="rd-dd">logical.  Should the legend be in the order of the levels of
<code>trace.factor</code> or in the order of the traces at their right-hand ends?</dd>
  <dt>xlab, ylab</dt>
  <dd class="rd-dd">the x and y label of the plot each with a sensible
default.</dd>
  <dt>ylim</dt>
  <dd class="rd-dd">numeric of length 2 giving the y limits for the plot.</dd>
  <dt>lty</dt>
  <dd class="rd-dd">line type for the lines drawn, with sensible default.</dd>
  <dt>col</dt>
  <dd class="rd-dd">the color to be used for plotting.</dd>
  <dt>pch</dt>
  <dd class="rd-dd">a vector of plotting symbols or characters, with sensible
default.</dd>
  <dt>xpd</dt>
  <dd class="rd-dd">determines clipping behaviour for the <code><a href=http://www.inside-r.org/r-doc/graphics/legend>legend</a></code> used,
see <code><a href=http://www.inside-r.org/r-doc/graphics/par>par</a>(xpd)</code>.  Per default, the legend is <em>not</em> clipped
at the figure border.</dd>
  <dt>leg.bg, leg.bty</dt>
  <dd class="rd-dd">arguments passed to <code><a href=http://www.inside-r.org/r-doc/graphics/legend>legend</a>()</code>.</dd>
  <dt>xtick</dt>
  <dd class="rd-dd">logical. Should tick marks be used on the x axis?</dd>
  <dt>xaxt, axes</dt>
  <dd class="rd-dd">graphics parameters to be passed to the plotting
routines.</dd>
  <dt>...</dt>
  <dd class="rd-dd">graphics parameters to be passed to the plotting routines.</dd>
</dl>

  <h4>Details</h4>

  <p class="rd-p">By default the levels of <code>x.factor</code> are plotted on the x axis in their
given order, with extra space left at the right for the legend (if
specified). If <code>x.factor</code> is an ordered factor and the levels are
numeric, these numeric values are used for the x axis.</p>

  <p class="rd-p">The response and hence its summary can contain missing values. If so, the
missing values and the line segments joining them are omitted from the plot
(and this can be somewhat disconcerting).</p>

  <p class="rd-p">The graphics parameters <code>xlab</code>, <code>ylab</code>, <code>ylim</code>, <code>lty</code>,
<code>col</code> and <code>pch</code> are given suitable defaults (and <code>xlim</code> and
<code>xaxs</code> are set and cannot be overridden).  The defaults are to cycle
through the line types, use the foreground colour, and to use the symbols
1:9, 0, and the capital letters to plot the traces.</p>


  <h4>Note</h4>

  <p class="rd-p">Some of the argument names and the precise behaviour are chosen for
S-compatibility.</p>


  <h4>References</h4>

  <p class="rd-p">Chambers, J. M., Freeny, A and Heiberger, R. M. (1992)
<em>Analysis of variance; designed experiments.</em> Chapter 5 of
...
<code><a href=http://www.inside-r.org/r-doc/stats/interaction.plot>interaction.plot</a></code>


<h4>Author</h4>

Originates in the <code>stats</code> package in the R distribution,
...
<h3>The Beta-Binomial Distribution</h3>

<p class="rd-p">Density, distribution function, quantile function, and random generation
for the beta-binomial distribution.  A beta-binomial variate
follows the binomial distribution with parameters
<code>N</code> and <code>p</code>, where the probability <code>p</code> of success itself has
a beta distribution with parameters <code>u</code> and <code>v</code>.</p>

<h4>Usage</h4>
<pre class="r"><code>dbb(x, N, u, v)
pbb(q, N, u, v)
qbb(p, N, u, v)
rbb(n, N, u, v)</code></pre>

<h4>Arguments</h4>
<dl class="rd-dl">
  <dt>x</dt>
  <dd class="rd-dd">vector of qauntiles</dd>
  <dt>N</dt>
  <dd class="rd-dd">number of trials (a positive integer)</dd>
  <dt>u</dt>
  <dd class="rd-dd">first positive parameter of the beta distribution</dd>
  <dt>v</dt>
  <dd class="rd-dd">second positive parameter of the beta distribution</dd>
  <dt>q</dt>
  <dd class="rd-dd">vector of quantiles</dd>
  <dt>p</dt>
  <dd class="rd-dd">vector of probabilities</dd>
  <dt>n</dt>
  <dd class="rd-dd">number of observations</dd>
</dl>

  <h4>Details</h4>

  <p class="rd-p">The beta-binomial distribution with parameters <em>N</em>, <em>u</em>, and
<em>v</em> has mass function given by <em>choose(N, x) * Beta(x + u, N - x + v) /
Beta(u, v)</em> for <em>u > 0</em>, <em>v > 0</em>, a positive integer <em>N</em>, and
any nonnegative integer <em>x</em>. Although one can express the integral in
...
the mass function.</p>
  ...
  <p class="rd-p">The mean, <em>mu</em>, and variance, <em>sigma^2</em>, of the beta-binomial distribution
can be computed explicitly as <em>mu = N * u / (u + v)</em> and
<em>sigma^2 = (N * u * v * (N + u + v)) / ((u + v)^2 * (1 + u + v))</em>.</p>

  <p class="rd-p"><code>dbb</code> and <code>qbb</code> do not recycle vector-valued arguments in
the usual fashion in R.  Arguments to these functions must have length 1
and/or one other common length. See example of <code><a href=#pcbinom>pcbinom</a></code> for more
information.</p>

  <p class="rd-p">Non-integer values of <code>x</code> and <code>N</code> are rounded to the nearest
integer with a warning.</p>


  <h4>Value</h4>

  <p class="rd-p"><dl>
<code>dbb</code> gives the density, <code>pbb</code> gives the distribution
function, <code>qbb</code> gives the quantile function, and <code>rbb</code> generates
random deviates.
</dl></p>
  ...
  <p class="rd-p"><code>dbb</code>, <code>pbb</code>, and <code>qbb</code> can be imprecise
when the probabilities for tail values are small.  This can result in
unusual behavior for <code>pbb</code> and <code>qbb</code>.</p>


  <h4>References</h4>

  <p class="rd-p"><a href = http://bioinformatics.mdanderson.org/Software/OOMPA/Current/TailRank-manual.pdf>http://bioinformatics.mdanderson.org/Software/OOMPA/Current/TailRank-manual.pdf</a></p>



<h4>Examples</h4>
<pre class="r"><code># set up parameters
w <- 10
u <- 0.3 *w
v <- 0.7 * w
N <- 12
...
# this to return 0:29
qbb(pbb(0:29, 29, 1, 33), 29, 1, 33)</code></pre>

<h4>See also</h4>

<code><a href=http://www.inside-r.org/r-doc/stats/Beta>dbeta</a></code> for the beta distribution and
<code><a href=http://www.inside-r.org/r-doc/stats/Binomial>dbinom</a></code> for the binomial distribution.
...
of the k binomial variates</dd>
  ...
  <dt>log, log.p</dt>
  <dd class="rd-dd">logical; if TRUE, probabilities p are given as log(p).
(See NOTE in details).</dd>
  <dt>verbose</dt>
...
code in <code>dkbinom.c</code> for more details.</dd>
  <dt>method</dt>
  <dd class="rd-dd">The <code>butler</code> (default) method is the
...
transform to compute the convolution of k binomial random variates.</dd>
  <dt>checkArgs</dt>
  <dd class="rd-dd"><code>=TRUE</code> checks the incoming arguments <code>x</code>, <code>size</code>,
<code>prob</code> for correct structure and consistency.</dd>
  <dt>q</dt>
...
binomials are used to calculate the mass or the distribution functions.</p>
  ...
  <p class="rd-p">NOTE: When <code>log.p</code> or <code>log</code> is <code>TRUE</code>, these functions do
...
distribution function.  Produces errors for invalid inputs of <code>size</code>,
<code>prob</code>, <code>x</code>, and <code>q</code>.
</dl></p>
...
<p class="rd-p">Wrapper for <code>smartFilter</code> that creates a set of symmetric weights for
...
<pre class="r"><code>movAvg2(y = NULL, bw = 30, type = c("gaussian", "exponential", "linear",
  "uniform"), furthest.weight = 0.01, center.weight = 1, plot = FALSE,
  ...)</code></pre>
...
calculated.  If <code>NULL</code>, the moving averages are not calculated--but this can
be useful for visualizing the weights. See the <code>plot</code> argument.</dd>
  <dt>bw</dt>
  <dd class="rd-dd">bandwidth, which is roughly half the width of the moving
window.  The total width of the window is <code>2 * bw + 1</code>.</dd>
  <dt>type</dt>
...
decay, or uniform weights. Defaults to gaussian.</dd>
  <dt>furthest.weight</dt>
  <dd class="rd-dd">The unormalized value of the weights at the left and
right edges of the window</dd>
  <dt>center.weight</dt>
  <dd class="rd-dd">The unnormalized value of the weights at the center of
the window</dd>
  <dt>plot</dt>
  <dd class="rd-dd"><code> = TRUE</code> produces a plot which shows the weights.</dd>
  <dt>...</dt>
  <dd class="rd-dd">additional arguments to <code><a href=#smartfilter>smartFilter</a></code></dd>
</dl>
...
weights as possible.</p>
  ...
  <p class="rd-p"><dl>
If <code>y</code> is not <code>NULL</code>, returns the moving average of the
series.  Also produces a plot of the weights if <code>plot = TRUE</code>.
</dl></p>
...
<dl class="rd-dl">
  ...
  <p class="rd-p"><dl>
In the sequence <code>x[1], x[2], ..., x[n]</code>, <code>cumMax</code>
returns the vector <code>y</code> such that for each <code>i = 1,...,n</code>,
<code>y[i] = max(x[j]; j = 1,...,i)</code>
</dl></p>
...
<h3>Computes the cummulative sum of a vector without propogating NAs</h3>

<p class="rd-p">Computes the cummulative sum of a vector without propogating NA's.</p>
...
point (double) addition is used.</p>


  <h4>Value</h4>

  <p class="rd-p"><dl>
The vector of cumulative sums.</p>

  <p class="rd-p">Elements in <code>x</code> that were <code>NA</code> will continue to be <code>NA</code>, but
the <code>NA</code> will not be propogated.
</dl></p>
...
center, to be used for illustration.</p>
